...
{ 
     ...
     on_update_callback: 'http://your-site/your-url'
}
...
+ on_update_callback - (необязательный параметр) - если через какое-то время наш отчет обновится - будет вызван этот URL запросом GET.
...
### 5. Послать письмо c pdf отчетом на e-mail (возможно несколько) по номеру заказа
...
### 6. Получить конечный JSON по номеру заказа
...
### 8. Получить ссылку на готовый pdf с отчетом по номеру заказа

```javascript
POST /v2/partners/:login/orders/:invId/pdf_report
